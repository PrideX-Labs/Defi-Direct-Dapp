...
const Sidebar = ({ isMobileMenuOpen, setIsMobileMenuOpen }: { isMobileMenuOpen: boolean; setIsMobileMenuOpen: Dispatch<SetStateAction<boolean>> }) => {
  ...
  useEffect(() => {
    const path = pathname === "/" ? "Home" : pathname?.split("/")[1] || "Dashboard";
    ...
    <>
      ...
      >
        ...
        <button
          ...
          className="hidden lg:flex absolute -right-3 top-20 bg-[#151021] p-2 rounded-full cursor-pointer border-2 border-[#9C2CFF]"
        >
          {isCollapsed ? <FaBars className="text-white" /> : <FaTimes className="text-white" />}
        </button>

        {/* Close Button - Mobile */}
        <button onClick={() => setIsMobileMenuOpen(false)} className="lg:hidden absolute right-4 top-4 text-white">
          <FaTimes size={24} />
        </button>
        ...
        {/* Navigation */}
        <nav className={cn("flex flex-col mt-[70px] gap-6", isCollapsed ? "px-2" : "pl-8")}>
          {menuItems.map((item) => (
            <Link
              key={item.name}
              href={item.link}
              className={cn(
                "flex h-[68px] gap-3 p-3 rounded-lg transition-all duration-200 items-center relative group",
            ...
            >
              ...
              {!isCollapsed ? (
                ...
                <span className="absolute left-full ml-4 px-2 py-1 bg-gray-800 rounded-md text-sm whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity shadow-lg">
                  {item.name}
                </span>
              )}
            </Link>
        ...
        <button onClick={() => setIsCollapsed(!isCollapsed)} className="lg:hidden mt-auto mb-8 mx-auto p-2 text-white">
